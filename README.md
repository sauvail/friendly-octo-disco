...
- **Programme (kanban)** — build blocs → semaines → séances → exercices on a **drag-and-drop board** (collapsible week sections, workout cards). Plan sets as `%1RM` or absolute load with reps & RPE. **Muscle wildcards** — plan a slot by muscle (e.g. *Biceps 3×12 @8*) and pick the exact exercise (filtered to that muscle) when you run the workout, or keep it generic. **Supersets** (link exercises A1/A2), **"bloc suivant"** auto-progression (+kg on reference 1RMs), and in-app rename/duplicate/delete with **undo**.

...
- **Éditer le programme (multi-column)** — the **"✎ Éditer le programme (vue colonnes)"** button on the board opens the program's séances as **side-by-side editable columns**, **one week at a time** with a **‹ Préc. / Suiv. ›** week navigator (`editAllWi`, showing `week i/N`). The columns **grow to fill the full viewport width** (`flex:1`, breaking out full-bleed); if a week has more séances than fit, the strip scroll-snaps horizontally. Each column has its own ▶ run and ⋯ menu, and **exercises can be dragged to reorder within their column**.

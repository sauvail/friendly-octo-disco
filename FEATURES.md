...
Item    = { id, exId, muscle?, group?, mods?, sets: [ Set ] }
...
Set (weight) = { id, load, reps, rpe, aLoad, aReps, aRpe, done, date }
Set (cardio) = { id, dur, dist, aDur, aDist, done, date }
  load/reps/rpe = planned; aLoad/aReps/aRpe = actual (logged); done + date set on completion.
```
...
- **Workout cards** — each séance card shows its name, a preview of its first exercises, and a **progress bar** (logged sets / total). Tap the card body to open its editor; the blue **▶** button runs it. `＋ Séance` adds a workout to that week.
...
Two ways to edit, sharing one renderer (`itemsEditorHtml(wi, di, …)`), so they always stay in sync:

- **Single séance** — tap a workout card. Header has the name, a **⋯ menu** (rename/duplicate/delete) and **▶ Démarrer la séance**. Exercises can be reordered by drag.
- **Tout éditer (multi-column)** — the **"✎ Tout éditer les séances"** button on the board opens *every* séance of the program as **side-by-side editable columns**. The strip breaks out to the **full viewport width**, so a wide screen shows them all at once; on mobile the columns are ~86 % wide with **scroll-snap** so you swipe between them. Each column has its own ▶ run and ⋯ menu.

Within an exercise you can: add/remove **séries**, edit **kg / reps / RPE** inline, add a **modifier** (§6), **link a superset** (§7), and delete the exercise (with undo). `＋ Exercice` opens the picker (§4).
...
- **Library editor** (Données → Bibliothèque): per exercise you set **name, category, metric (charge×reps / cardio), competition lift, and muscle**. Add or delete exercises.
...
## 5. Set planning (absolute kg)

Sets are planned in **absolute kg + reps + RPE**. (The previous **%1RM / "1RM de référence" mechanic was removed**; `orm` is always `0` and no longer shown. Any old `%1RM` plans were **baked into absolute loads on upgrade** by the migration, so nothing was lost.) Cardio exercises plan **duration (min) + distance (km)** instead.
...
Start from a card's **▶** (or **Démarrer la séance** in the editor). The run view focuses on logging:
...
- **Rest timer** — checking a set off **auto-starts** a countdown (default 120 s; `−15 / +15 / Passer`), with **vibration + beep** at zero.
...
Cardio runs log **min / km** against the planned targets.
...
Computed from your **logged** sets (estimated 1RM via **Epley**: `load × (1 + reps/30)`).
